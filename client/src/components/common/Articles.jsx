...
function Articles() {
    ...
    const verifyUserRole = async () => {
      if (user?.emailAddresses?.length > 0) {
        try {
          ...
          let response = await axios.post('${import.meta.env.local.URL}/author-api/author', userData, {
          ...
          if (response.data.message === "Invalid role") {
            userData.role = 'user';
            response = await axios.post('${import.meta.env.local.URL}/user-api/user', userData, {
              headers: { Authorization: `Bearer ${token}` }
            });
          }
  ...
  async function getArticles() {
    try {
      const token = await getToken();
      const res = await axios.get('${import.meta.env.local.URL}/author-api/articles', {
        headers: {
